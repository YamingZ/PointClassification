...
def uniformSampling(file_names, NUM_POINT):
    ...
    for index in range (len(file_indexs)):
        ...
        for i,object_xyz in enumerate (current_data):
            samples_index=np.random.choice(2048, NUM_POINT, replace=False) #随机从2048个点中取NUM_POINT个点的ID
...
def load_data(NUM_POINT, sampleType, data_dir):
...
def prepareGraph(inputData, neighborNumber, pointNumber, dataType, graph_dir):
    ...
    # baseDir = '/opt/PycharmProjects/data/'
...
def prepareData(inputTrain, inputTest, neighborNumber, pointNumber, graph_dir):
    ...
    scaledLaplacianTrain = prepareGraph(inputTrain, neighborNumber, pointNumber, 'train', graph_dir)
    scaledLaplacianTest = prepareGraph(inputTest, neighborNumber, pointNumber, 'test', graph_dir)
